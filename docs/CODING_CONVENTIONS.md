...
- 命令文件与回执文件**双向都原子写**（同目录临时文件 + rename/replace 落位）；
  扩展对成功回执做 requestId 幂等重放缓存（5 分钟/50 条），重试复用 requestId 不重复执行副作用。
...
- 仓库根目录 `tmp/` 用于放置**临时/一次性**文件（测试产物、调试快照、中间脚本等）；
  已被 `.gitignore` 忽略（保留 `tmp/.gitkeep` 占位以便克隆后目录存在）。
- **临时文件使用完后应及时清除**：会话结束或调试结束即删除；不得遗留在 `tmp/` 长期占用。
- 编码门禁（`tools/enforce_encoding.py`）会跳过 `tmp/`；但清理职责仍属使用者。
